...
export default {
   ...
   * 将请求参数拼装到url中
   */
  convertRealUrl(url: string, reqParamObj: object) {
    let tmpUrl = url;
    if (tmpUrl.indexOf('?') !== -1) {
   ...
   * @returns string
   */
  formatMessage(langMessage: object, messageFlag: string, params: object) {
    // @ts-ignore
   ...
   * @returns string
   */
  formReplacePathVar(url: string, params: object) {
    let tmpUrl = url;
    if (typeof (params) !== 'undefined' && params !== null) {
      for (const propName in params) {
        // @ts-ignore
   ...
   * @returns boolean
   */
  arrayLikeContainsVal(arr: Array<string>, val: any) {
   ...
   * @returns {array} array
   */
  removeAllMatchedValInArray(arr = [], val: string) {
    let len = arr.length;
    while (len--) {
      if (val === arr[len]) {
        arr.splice(len, 1);
   ...
   * @param {string} val
   */
  setJsSessionItem(key: string, val: string) {
   ...
   * @param {object}} jsonObj
   */
  jsonObj2StrFormat(jsonObj: object) {
   ...
   */
  dateFormat(date: Date, patternStr: string) {
    let fmt = patternStr;
    var o = {
